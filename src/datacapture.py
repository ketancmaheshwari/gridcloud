...
class clientThread(threading.Thread):
	def __init__(self, ip, port):
		...
		self.asock=socket.socket(socket.AF_INET, socket.SOCK_STREAM)
                self.asock.settimeout(DEFAULT_TIMEOUT) # has no effect...

                while True:
                    try:
                        self.asock.connect((self.ip, self.port)) # connect to server
                        break
                    except socket.error:
                        time.sleep(RECONNECT_INTERVAL)
                        print 'server not ready'

		self.ca_cert_path=os.path.join(os.path.dirname(sys.argv[0]),'cert.pem')
		self.sock=ssl.wrap_socket(self.asock, ssl_version=ssl.PROTOCOL_SSLv3, cert_reqs=ssl.CERT_REQUIRED, ca_certs=self.ca_cert_path)
	...
	def run(self):
		...
		try:
			while True:
                                ...
                                if not data: # empty string->connection lost
                                    self.asock.shutdown(socket.SHUT_RDWR)
                                    self.asock.close()
                                    self.asock=socket.socket(socket.AF_INET, socket.SOCK_STREAM)
                                    while True:
                ...
                # doesn't work 
                #except socket.timeout: # assume pmu server went down
                #        print 'hello'
